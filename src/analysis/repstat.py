...
class RepStat:
    ...
    def __init__(
        ...
        for metric in ["test_performance_1", "test_performance_2"]:
            for ablation in AB_STAT:  # ["rand", "stat"]
        ...
        repstat_df["significant"] = repstat_df.apply(
            lambda row: (
                np.nan if row.drop("significant").isnull().any() else row["significant"]
            ),
            axis=1,
        )
    ...
    def _get_reptest(self, metric: str, ablation: str) -> pd.DataFrame:
        ...
        emb_df = (
            self.df[
                ...
                & (self.df["ptp"] == 1)
                & (self.df["max_seq_num"] == 200)
            ]
            .drop(columns=["ablation", "value", "embseed", "max_seq_num"])
        ...
        merge_df = pd.merge(
            ab_df,
            emb_df,
            on=["arch", "task", "model", "metric", "sampleseed"],
            how="left",
        ).dropna()
        ...
        # df = merge_df.groupby(["arch", "task", "model", "metric", "sampleseed"])

        tested_df = (
            merge_df.groupby(["arch", "task", "model", "metric", "sampleseed"])
    ...
    def df(self):
        ...
        df["last_layer"] = (
            df["value"].apply(lambda x: ast.literal_eval(x)[-1] if x else None)
            # .replace(0, np.nan)
        )
        return df.copy()
