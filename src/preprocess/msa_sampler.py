...
from src.models.protxlstm.utils import AA_TO_ID
...
class MSASampler:
    ...
    def _get_sim_filtered_idxs(self, msa: np.ndarray) -> np.ndarray:
        norm_sim = self.get_similarity(msa)
        dsim = 1 - norm_sim
    ...
    def get_similarity(self, msa: np.ndarray) -> np.ndarray:
    ...
    def get_sample_idxs(
        ...
        if self.force_include_first:
            # Ensure the first index is included
            sample_idxs = np.concatenate([[0], sample_idxs[sample_idxs != 0]])
            
        # Filter weights to match valid indices and normalize them
        weights = weights[sample_idxs]
        weights = weights / weights.sum() 

        # Sample based on filtered indices and weights
        return (
            np.random.choice(
                sample_idxs,
                replace=False,
            ...
            if len(sample_idxs) >= size else sample_idxs
        )
...
def sample_msa(
    ...
    sampler = MSASampler(max_similarity, max_dissimilarity, force_include_first=force_include_first)
