...
from src.params.aa import AA_NUMB
from src.params.sys import RAND_SEED, DEVICE
...
from src.preprocess.data_process import split_protrain_loader, DatasetInfo
...
from src.probing.pytorch_model import (
    LinearRegression,
    LinearClassifier,
)

from src.probing.train_test import train, test
from src.vis.learning_vis import plot_lc
from src.utils import checkNgen_folder, get_folder_file_names, pickle_save, seed_all
...
class Run_Pytorch:
    def __init__(
        ...
        test_embed_folder: str | None = None,
        ...
        **encoder_params,
        ...
        if self._encoder_name not in (
            ...
            + list(CARP_INFO.keys())
            + list(PROTXLSTM_INFO.keys())
            + list(PROTMAMBA_INFO.keys())
            + list(POET_INFO.keys())
        ):
        ...
        self._test_embed_folder = (
            embed_folder if test_embed_folder is None else test_embed_folder
        )
        ...
        if self._checkpoint != 1:
            ...
            self._test_embed_folder += f"-{str(self._checkpoint)}"
        ...
        if (
            isinstance(self._reset_param, str)
            and f"-{self._reset_param}" not in self._all_result_folder
        ):
            self._all_result_folder += f"-{self._reset_param}"
            self._all_plot_folder += f"-{self._reset_param}"
        ...
        if encoder_class == ESMEncoder:
        ...
        elif encoder_class in [ProtxLSTMEncoder]:
            self._encoder_info_dict = PROTXLSTM_INFO
            self._all_result_folder = checkNgen_folder(
                os.path.join(
                    self._all_result_folder, f"sampling_seed-{str(sampling_seed)}"
                )
            )
            self._all_plot_folder = checkNgen_folder(
                os.path.join(
                    self._all_plot_folder, f"sampling_seed-{str(sampling_seed)}"
                )
            )
        elif encoder_class in [ProtMambaEncoder]:
            self._encoder_info_dict = PROTMAMBA_INFO
            self._all_result_folder = checkNgen_folder(
                os.path.join(
                    self._all_result_folder, f"sampling_seed-{str(sampling_seed)}"
                )
        ...
        elif encoder_class in [PoETEncoder]:
            self._encoder_info_dict = POET_INFO
            self._all_result_folder = checkNgen_folder(
                os.path.join(
                    self._all_result_folder, f"sampling_seed-{str(sampling_seed)}"
                )
            ...
            # TODO aultoto
            if self._flatten_emb == False or self._flatten_emb == "mean":
                self._encoder_info_dict = {
                    "onehot": (AA_NUMB,)
                }  # embedding dim => [batch_size, seq_len, aa_num] | [batch_size, aa_num]
            else:
                self._encoder_info_dict = {
                    "onehot": (MAX_SEQ_LEN * AA_NUMB,)
                }  # embedding dim => [batch_size, seq_len*aa_num]
        ...
        self.yield_with_context = yield_with_context
        self.max_context_length = max_context_length
        self.max_context_sequences = max_context_sequences
    ...
    def run_pytorch_layer(self, embed_layer):
        ...
        self._loader_dict = split_protrain_loader(
            ...
            test_embed_folder=self._test_embed_folder,
            embed_layer=embed_layer,
            ...
            yield_with_context=self.yield_with_context,
            max_context_length=self.max_context_length,
            max_context_sequences=self.max_context_sequences,
        ...
        for subset, loader_key in zip(self._subset_list, self._loader_dict.keys()):
            ...
            if model_name == "LinearRegression":
                ...
                pred_prob = nn.Softmax(dim=1)(torch.from_numpy(pred)).numpy()
                calc_roc_auc = pred_prob.shape[-1] < 100
                ...
                result_dict[subset] = {
                    ...
                    "top5_acc": (
                        top_k_accuracy_score(
                            true, pred_prob, k=5, labels=np.arange(pred.shape[-1])
                        )
                        if not calc_roc_auc
                        else np.nan
                    ),
                ...
                if subset in ["train", "val"]:
                    print(f"loss: {loss}")
                    print(f"top-1 acc: {accuracy_score(true, cls)}")
                    print(
                        f"top-5 acc: {top_k_accuracy_score(true, pred_prob, k=5, labels=np.arange(pred.shape[-1]))}"
                    )
