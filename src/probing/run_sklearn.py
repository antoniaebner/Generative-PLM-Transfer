...
from src.utils import get_folder_file_names, pickle_save, ndcg_scale, checkNgen_folder, seed_all
...
from src.params.sys import RAND_SEED, SKLEARN_ALPHAS
from src.preprocess.data_process import ProtranDataset, DatasetInfo
    ...
    def __init__(
        ...
        **encoder_params,
        ...
        self.test_dataset_path = (
            dataset_path if test_dataset_path is None else test_dataset_path
        )
        ...
        if self.reset_param and "-rand" not in self.embed_folder:
            ...
            self.test_embed_folder += "-rand"
        if (
            isinstance(self.reset_param, str)
            and f"-{self.reset_param}" not in self.all_result_folder
        ):
            self.all_result_folder += f"-{self.reset_param}"
        if (
            isinstance(self.reset_param, str)
            and f"-{self.reset_param}" not in self.embed_folder
        ):
            self.embed_folder += f"-{self.reset_param}"
            self.test_embed_folder += f"-{self.reset_param}"
        ...
        elif self.encoder_name in PROTXLSTM_INFO.keys():
            total_emb_layer = PROTXLSTM_INFO[encoder_name][1] + 1
            self.all_result_folder = checkNgen_folder(
                os.path.join(
                    self.all_result_folder, f"sampling_seed-{str(sampling_seed)}"
                )
            )
        elif self.encoder_name in PROTMAMBA_INFO.keys():
        ...
        elif self.encoder_name in POET_INFO.keys():
        ...
        print(f"self.test_embed_folder: {self.test_embed_folder}")
        ...
        all_sklearn_results = {}
        start_layer = 0 if self.all_embed_layers else total_emb_layer - 1

        for layer in range(start_layer, total_emb_layer):
            all_sklearn_results[layer] = self.run_sklearn_layer(
                embed_layer=layer,
            )
    ...
    def pick_model(
        ...
        for alpha in self.alphas:
            ...
            model = make_pipeline(
                ...
                self.sklearn_model(
                    random_state=self.sklearn_state,
                    solver="auto",
                    **self.sklearn_params,
                ),
            )
