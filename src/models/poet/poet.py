...
from src.models.poet.modules.activation import gelu
from src.models.poet.modules.attention import MultiheadAttention
from src.models.poet.modules.embedding import RotaryEmbedding
from src.models.poet.modules.packed_sequence import (
    PackedTensorSequences,
    get_mask,
    pad_input,
    unpad_input,
)
from src.models.poet.modules.transformer import TransformerEncoder
from src.models.poet.modules.transformer_rotary import TieredRotaryTransformerEncoderLayer
...
def _apply_causal_prefix_attention(
    ...
    for layer_idx, layer in enumerate(decoder.layers):
        ...
        if layer_idx in save_layer:
            representations[layer_idx] = x_norm.x.view(B, L_x, -1)
...
class PoET(nn.Module, LogitsAllocateMemoryMixin):
    ...
    def logits(
        ...
        if save_layer:
            x, representations = _apply_causal_prefix_attention(
        ...
        if len(self.decoder) in save_layer:
            representations[len(self.decoder)] = embeddings.view(B, L_x, -1)

        logits = self.linear.forward(embeddings).view(B, L_x, -1)
        if save_layer:
            return logits, representations
        elif not return_embeddings:
            return logits
        else:
