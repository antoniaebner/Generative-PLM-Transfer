...
class Block(nn.Module):
    def __init__(
        ...
        Adaptation by Antonia Ebner: get hiddens after layernorm
        """
        super().__init__()
    ...
    def apply_norm(
        ...
        r"""Pass the input through the norm function of the layer.
...
class CheckpointedModule(torch.nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        return checkpoint(self.ckpt_layer, x, use_reentrant=False, **kwargs)
...
class MixerModelWithPosids(nn.Module):
    ...
    def forward(self, input_ids, position_ids, inference_params=None, save_layer=[]):
        ...
        if len(save_layer) > 0:
            hidden_states_dict = {}
        if 0 in save_layer:
            hiddens, _ = self.layers[0].apply_norm(hidden_states)
            hidden_states_dict[0] = hiddens.detach().cpu().to(torch.float).numpy()
        for i, layer in enumerate(self.layers):         
            hidden_states, residual = layer(
                hidden_states, residual, inference_params=inference_params
            )
            if i + 1 in save_layer:
                if i == len(self.layers) - 1:
                    ...
                    # apply layer norm of next block
                    hiddens, _ = self.layers[i+1].apply_norm(hidden_states, residual)

                    hidden_states_dict[i + 1] = (
                        hiddens.detach().cpu().to(torch.float).numpy()
                    )
