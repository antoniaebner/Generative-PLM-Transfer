...
#   - Return layer representations after layernorm
...
class xLSTMBlock(nn.Module):
    ...
    def __init__(self, config: xLSTMBlockConfig) -> None:
        ...
        if self.config.feedforward is not None: # is hardcoded as None in block.py!
    ...
    def forward(self, x: torch.Tensor, state=None, return_embedding=False, **kwargs) -> torch.Tensor:
        normed_x = self.xlstm_norm(x).to('cpu')

        if self.config.mlstm.return_last_state:
            x_xlstm, xlstm_state = self.xlstm(normed_x.to(next(self.parameters()).device), state=state, **kwargs)
            x = x + x_xlstm
        else:
            x = x + self.xlstm(normed_x.to(next(self.parameters()).device), state=state, **kwargs)

        if self.ffn is not None:
            x = x + self.ffn(self.ffn_norm(x), **kwargs)

        if self.config.mlstm.return_last_state and return_embedding:
            return x, xlstm_state, normed_x.detach().to('cpu')
        elif self.config.mlstm.return_last_state:
            return x, xlstm_state
        elif return_embedding:
            return x, normed_x.detach().to('cpu')
        else:
            return x
