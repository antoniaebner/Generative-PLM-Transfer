...
# Modified code from Prot-xLSTM under Apache License 2.0.

# Modified by Antonia Ebner, 2026:
#   - Return layer representations
...
class xLSTMBlockStack(nn.Module):
    ...
    def forward(self, x: torch.Tensor, state=None, save_layer=[], **kwargs) -> torch.Tensor:
        ...
        for block_idx, block in enumerate(self.blocks):
            ...
            return_embedding = len(save_layer) > 0 and block_idx in save_layer
            
            if self.config.mlstm_block.mlstm.return_last_state:

                if self.config.checkpoint_blocks and return_embedding:
                    x, new_state[f"block_{block_idx}"], layer_rep[block_idx] = checkpoint(block, x, state=block_state, use_reentrant=False, return_embedding=True, **kwargs)
                elif self.config.checkpoint_blocks:
                    x, new_state[f"block_{block_idx}"] = checkpoint(block, x, state=block_state, use_reentrant=False, **kwargs)
                elif return_embedding:
                    x, new_state[f"block_{block_idx}"], layer_rep[block_idx] = block(x, state=block_state, return_embedding=True, **kwargs)
                else:
                ...
                if self.config.checkpoint_blocks and return_embedding:
                    x, layer_rep[block_idx] = checkpoint(block, x, state=block_state, use_reentrant=False, return_embedding=True, **kwargs)
                elif self.config.checkpoint_blocks:
                    x = checkpoint(block, x, state=block_state, **kwargs, use_reentrant=False)
                elif return_embedding:
                    x, layer_rep[block_idx] = block(x, state=block_state, return_embedding=True, **kwargs)
                else:
        ...
        if return_embedding:
            layer_rep[block_idx+1] = x.detach().to('cpu')
        else:
            layer_rep = None

        if self.config.mlstm_block.mlstm.return_last_state:
            return x, new_state, layer_rep
        else:
            return x, layer_rep
