...
class xLSTMLMModel(WeightDecayOptimGroupMixin, nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, state=None, save_layer=[], **kwargs) -> torch.Tensor:
        ...
        if self.config.mlstm_block.mlstm.return_last_state:
            x, state, layer_reps = self.xlstm_block_stack(x, state=state, save_layer=save_layer, **kwargs)
        else:
            x, layer_reps = self.xlstm_block_stack(x, state=state, save_layer=save_layer, **kwargs)
        
        logits = self.lm_head(x)

        if self.config.mlstm_block.mlstm.return_last_state:
            return logits, state, layer_reps
        else:
            return logits, layer_reps
