# Original code from Prot-xLSTM under Apache 2.0 License.

# Modified by Antonia Ebner, 2026:
#   - add workable save_layer 
...
from src.models.protxlstm.generation import GenerationMixinSafe
from src.models.protxlstm.utils import load_config_hf, load_state_dict_hf
from src.models.protxlstm.xlstm.xlstm_lm_model import xLSTMLMModel, xLSTMLMModelConfig
...
class xLSTMLMHeadModel(nn.Module, GenerationMixinSafe):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if len(save_layer) > 0:
            save_layer = torch.Tensor(save_layer)
            save_layer = torch.where(save_layer >= 0, save_layer, save_layer + self.config.config_dataclass.num_blocks + 1)
            assert (save_layer <= self.config.config_dataclass.num_blocks).all()

        if self.config.config_dataclass.mlstm_block.mlstm.return_last_state:
            lm_logits, state, layer_embs = self.backbone(input_ids, position_ids=position_ids, seq_position_ids=seq_position_ids, state=state, save_layer=save_layer)
            CausalLMOutput = namedtuple("CausalLMOutput", ["loss", "logits", "state"])
            return CausalLMOutput(loss=None, logits=lm_logits, state=state), layer_embs
        else: 
            lm_logits, layer_embs = self.backbone(input_ids, position_ids=position_ids, seq_position_ids=seq_position_ids, state=state, save_layer=save_layer)
            CausalLMOutput = namedtuple("CausalLMOutput", ["loss", "logits"])
            return CausalLMOutput(loss=None, logits=lm_logits), layer_embs
